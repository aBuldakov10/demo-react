...
const LocationNotification = () => {
  ...
  const { coordinates, errorMessage, enabledInBrowser } =
    ...
    <Box sx={{ position: 'relative', zIndex: 1, opacity: 0.8 }}>
      ...
      {enabledInBrowser ? (
        ...
        >
          ...
          Your location is: latitude <strong>{coordinates.lat}</strong> and
          longitude <strong>{coordinates.lon}</strong>. The nearest weather data
          location is <strong>{name}</strong>.
        </Alert>
      ) : (
        <Alert
          severity="warning"
